...
SkapaStapelDiagram <- function(skickad_df, 
  ...
  stodlinjer_list <- Berakna_varden_stodlinjer(min_varde =  min(plot_df["total"]), max_varde = max_varde_plot_df, y_borjar_pa_noll = y_axis_borjar_pa_noll)
  ...
  if (!is.na(x_var_fokus)) {
    ...
    p <- plot_df %>% ggplot(aes(x=!!x_var, y=total)) + geom_bar(position = "dodge", stat="identity", aes(fill = !!x_var_fokus_asname))
  } else if (is.na(skickad_x_grupp)) {
    p <- plot_df %>% ggplot(aes(x=!!x_var, y=total, fill = chart_col))
  } else {
  ...
  p <- p +
    {if (antal_grupper > 3) {
      geom_bar(stat="identity",width = 0.6, position = position_dodge(0.6))
      #scale_x_discrete()
    ...
    #geom_bar(stat="identity", position = "dodge") +      #position = "dodge", width = 0.25,
      {if(dataetiketter){
        geom_text(aes(y=total+sign(total),label=total,
...
SkapaLinjeDiagram <- function(skickad_df, 
  ...
  if (berakna_index){
    ...
    y_titel <- paste0(strsplit(skickad_y_var, ",")[[1]][1], ", index 100 = ", min(plot_df$år)) 
  ...
  stodlinjer_list <- Berakna_varden_stodlinjer(min_varde =  min(plot_df["total"]), max_varde = max(plot_df["total"]))
  ...
  if (is.na(skickad_x_grupp)) {
    ...
    if (!diagram_facet) legend_pos <- "bottom" else legend_pos <- "none"
  }
  ...
  p <- p +
    ...
    scale_y_continuous(breaks = seq(min_yvar, max_yvar, 
                                    by = maj_by_yvar),
                       minor_breaks = seq(min_yvar, max_yvar, by = min_by_yvar),
                       labels = etikettformat,
                       expand = c(0,0), limits = c(limit_min,max_yvar)) +
