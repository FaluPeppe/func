...
LasInFilerOchFlikar <- function(url_xlsx) {
  ...
  # skapa progress bar 
  pb <- txtProgressBar(min = 1, max = ifelse(length(url_xlsx)==1 , 2, length(url_xlsx)), style = 3)
  ...
  for (i in 1:length(url_xlsx)){                                           
    tryCatch({
      ...
      for (j in 1:length(sheet)){ 
        ...
        tmp <- read_excel(url_xlsx[i], sheet=j, .name_repair = "minimal")
